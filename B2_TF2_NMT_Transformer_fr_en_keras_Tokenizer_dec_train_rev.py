...
for epoch in range(50):
    ...
    with tqdm_notebook(total=len(dataset), desc=f"Train {epoch+1}") as pbar:
        for (batch, (inp, tar)) in enumerate(dataset):
            ...
            """
            decrement = -8
            for idx_dec in range(decoder_len, 1, decrement):

                tar_inp  = tmp_tar_inp[:, :idx_dec]
                tar_real = tmp_tar_real[:, :idx_dec]
            ...
            enc_padding_mask, combined_mask, dec_padding_mask = create_masks(inp, tmp_tar_inp)
            '''
            2. Forward Pass
            '''
            predictions_tmp, _ = model(inp, tmp_tar_inp,
                True,
                enc_padding_mask,
                combined_mask,
                dec_padding_mask
            )
            train_accuracy(accuracy_function(tmp_tar_real, predictions_tmp))
