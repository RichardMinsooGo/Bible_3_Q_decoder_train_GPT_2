...
class DecoderLayer(tf.keras.layers.Layer):
    def __init__(self, pf_dim, hid_dim, n_heads, dropout):
        ...
        self.layernorm_dec_inp = tf.keras.layers.LayerNormalization(epsilon=1e-6)
        # self.layernorm_enc_out = tf.keras.layers.LayerNormalization(epsilon=1e-6)
    ...
    def call(self, inputs, training, look_ahead_mask):
        ...
        inputs     = self.layernorm_dec_inp(inputs)
        # enc_output = self.layernorm_enc_out(enc_output)
...
class GPT2(tf.keras.Model):
    ...
    def call(self, inp, training, look_ahead_mask):
        ...
        # 1. "inp" is the decoder input of tokenized datasets
        # 2. "look_ahead_mask" are created from decoder inputs, it is given to the "Key" and "Value" of Multi-Head Attention 1.
        dec_output, attention_weights = self.decoder(inp, training, look_ahead_mask)

        # 3. decoder output is creadted from decoder model

        final_output = self.fin_output(dec_output)

        # 4. Final outputs are created. Then it is used or Language Model. In the official tutorial "Softmax"  was missed.
...
for epoch in range(0, 100):
    ...
    with tqdm_notebook(total=len(dataset), desc=f"Train {epoch+1}") as pbar:
        for (batch, (inp, tar)) in enumerate(dataset):
            ...
            inp_zeros = tf.zeros_like(inp)

            ground_truth = tf.concat([inp_zeros, tmp_tar_real], axis = 1)
            ...
            with tf.GradientTape() as tape:
                ...
                for idx_dec in range(1, decoder_len, increment):

                    dec_inp     = tmp_tar_inp[:, :idx_dec]
                    dec_out_tmp = tmp_tar_real[:, :idx_dec]

                    # print(dec_inp)
                    enc_den_inp = tf.concat([inp, dec_inp], axis = 1)
                    # print(enc_den_inp)

                    dec_out     = tf.concat([inp_zeros, dec_out_tmp], axis = 1)
                    # print(dec_out)

                    combined_mask = create_masks(enc_den_inp)
                    # print('Step 1')
                    
                    '''
                    2. Forward Pass
                    '''
                    predictions, _ = model(enc_den_inp, True, combined_mask)
            ...
            enc_den_inp = tf.concat([inp, tmp_tar_inp], axis = 1)
            combined_mask = create_masks(enc_den_inp)
            predictions_tmp, _ = model(enc_den_inp, True, combined_mask)
            train_accuracy(accuracy_function(ground_truth, predictions_tmp))
...
def evaluate(text):
    ...
    enc_den_inp = tf.concat([encoder_input, output], axis = 1)
    print("enc_den_inp   : " , enc_den_inp)
    # print( "END_TOKEN[0] : ", END_TOKEN[0] )

    # Decoder's prediction starts
    for i in range(decoder_len):
        combined_mask = create_masks(enc_den_inp)

        predictions, attention_weights = model(enc_den_inp, False, combined_mask)
        ...
        enc_den_inp = tf.concat([enc_den_inp, predicted_id], axis=-1)
